async function fetchData() {
    try {
                ...
                <div class="card_img-block">
                    ...
                    <div class="card_img_blackout">
                        <div class="add_to_cart_button">
                            <svg class="add_to_cart_button-img" xmlns="http://www.w3.org/2000/svg" width="27" height="25" viewBox="0 0 27 25" fill="none">
        ...
        //функция отправки товаров в корзину
        const buttonsForAddingToCart = document.querySelectorAll('.add_to_cart_button');
        buttonsForAddingToCart.forEach((el) => {
            el.addEventListener('click', () => {
                const itemForAddingToCart = el.closest('.product__cards__card');

                // с карточки товара получает имя, ссылку на изображение, цену
                const name = itemForAddingToCart.querySelector('.card_title').innerHTML;
                const imgTag = itemForAddingToCart.querySelector('.card_img');
                const image = imgTag.attributes[0].textContent;
                const price = itemForAddingToCart.querySelector('.price_value').innerHTML;
                const size = itemForAddingToCart.querySelector('.info-block-size').innerHTML;
                const color = itemForAddingToCart.querySelector('.info-block-color').innerHTML;

                const cartItem = `
                    <div class="cart_item">
                        <div class="cart_item-img-box">
                            <img src="${image}" alt="img" class="cart_item-img">
                        </div>
                        <div class="cart_item_description">
                            <div class="cart_item-description-heading">${name}</div>
                            <div class="cart_item_description-content">
                                <p class="cart_item_price">Price: <span class="price-currency">$</span><span class="price-value">${price}</span></p>
                                <p class="cart_item_color">Color: <span class="color-value">${color}</span></p>
                                <p class="cart_item_size">Size: <span class="size-value">${size}</span></p>
                                <form action="#" class="cart_item_quantity">
                                    <label for="item_quantity_value">Quantity:</label>
                                    <input type="text" name="item_quantity_value" class="item_quantity_value_input-area">
                                </form>
                            </div>
                        </div>
                ...
                removeButtons.forEach((el) => {
                    el.addEventListener('click', () => {
                        const itemForRemoving = el.closest('.cart_item');
                        itemForRemoving.remove();
                    })
                })
            })
        })
